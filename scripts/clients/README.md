...
See [configs/clients/claude/](../../configs/clients/claude/) for unified bridge setup.
...
echo '{"jsonrpc":"2.0","method":"tools/list","id":1}' | scripts/clients/mcp-stdio-bridge.sh
```
...
python3 scripts/clients/generate-claude-config.py --dry-run

# Validate output
python3 scripts/clients/generate-claude-config.py --dry-run | jq .
```
